...
extern "C" __global__ void computeGridForce(
    ...
    if (isInside) {
        ...
        if (invPower > 0.0f) {
            float baseInterpolated = ox * vm + fx * vp;
    ...
    atomicAdd(&forceBuffers[index], (unsigned long long)(atomForce.x * 0x100000000));
    atomicAdd(&forceBuffers[index + paddedNumAtoms], (unsigned long long)(atomForce.y * 0x100000000));
    atomicAdd(&forceBuffers[index + 2 * paddedNumAtoms], (unsigned long long)(atomForce.z * 0x100000000));
