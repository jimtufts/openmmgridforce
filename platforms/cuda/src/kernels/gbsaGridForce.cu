...
/**
 * Compute receptor desolvation energy and forces.
 * The receptor desolvation grid stores energy values computed with a probe radius.
 * For atoms with different scaled radii, we apply S³ scaling:
 *   E_atom = E_grid * (S_atom / S_probe)³
 *
 * This accounts for the fact that the desolvation energy scales approximately
 * with the cube of the solvent-excluded volume.
 *
 * @param posq                  Positions (xyz) and charges (w)
 * @param particleIndices       Which particles to process
 * @param radii                 Intrinsic radii (template)
 * @param scaleFactors          OBC scale factors (template)
 * @param gridCounts            Grid dimensions [nx, ny, nz]
 * @param gridReceptorDesolv    Receptor desolvation energy grid
 * @param gridReceptorDesolvDerivs  Derivatives for tricubic/triquintic (or nullptr)
 * @param groupStart            Group start indices
 * @param numGroups             Number of particle groups
 * @param originX/Y/Z           Grid origin
 * @param gridSpacing           Uniform grid spacing (nm)
 * @param receptorDesolvProbeRadius  Probe radius used to generate the grid
 * @param totalParticles        Total number of particles to process
 * @param templateNumAtoms      Number of atoms in ligand template
 * @param interpolationMethod   0=trilinear, 1=bspline, 2=tricubic, 3=triquintic
 * @param hasDerivatives        Whether gridReceptorDesolvDerivs is valid
 * @param groupReceptorEnergies Output: receptor desolvation energy per group
 * @param forceBuffer           Output: force accumulation buffer
 * @param paddedNumAtoms        Padded atom count for force buffer layout
 */
extern "C" __global__ void computeReceptorDesolvation(
    const float4* __restrict__ posq,
    const int* __restrict__ particleIndices,
    const float* __restrict__ radii,
    const float* __restrict__ scaleFactors,
    const int* __restrict__ gridCounts,
    const float* __restrict__ gridReceptorDesolv,
    const float* __restrict__ gridReceptorDesolvDerivs,
    const int* __restrict__ groupStart,
    int numGroups,
    float originX, float originY, float originZ,
    float gridSpacing,
    ...
    int hasDerivatives,
    float* __restrict__ groupReceptorEnergies,
    unsigned long long* __restrict__ forceBuffer,
    int paddedNumAtoms
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // Get atom's scaled radius
    float R_i = radii[templateIdx];
    float R_i_off = R_i - DIELECTRIC_OFFSET;
    float S_i = R_i_off * scaleFactors[templateIdx];

    // Compute S³ scaling factor
    // The grid was computed with S_probe, we scale for S_atom
    float S_probe_off = receptorDesolvProbeRadius - DIELECTRIC_OFFSET;
    float S_ratio = S_i / S_probe_off;
    float S_cubed = S_ratio * S_ratio * S_ratio;

    // Set up grid spacing array for interpolation functions
    float gridSpacingArr[3] = {gridSpacing, gridSpacing, gridSpacing};

    // Interpolate receptor desolvation grid
    InterpolationResult result;

    // Choose interpolation method
    if (interpolationMethod == 2 && hasDerivatives) {
        // Tricubic interpolation
        result = tricubicInterpolate(
            gridReceptorDesolv, gridReceptorDesolvDerivs, gridCounts, gridSpacingArr,
            originX, originY, originZ, position, true, true);
    } else if (interpolationMethod == 3 && hasDerivatives) {
        // Triquintic interpolation
        result = triquinticInterpolate(
            gridReceptorDesolv, gridReceptorDesolvDerivs, gridCounts, gridSpacingArr,
            originX, originY, originZ, position, true, true);
    } else if (interpolationMethod == 1) {
        // B-spline interpolation
        result = bsplineInterpolate(
            gridReceptorDesolv, gridCounts, gridSpacingArr,
            originX, originY, originZ, position, true, true);
    } else {
        // Trilinear interpolation (default)
        result = trilinearInterpolate(
            gridReceptorDesolv, gridCounts, gridSpacingArr,
            originX, originY, originZ, position, true, true);
    }

    if (!result.isInside) {
        return;  // Outside grid, no contribution
    }

    // Apply S³ scaling to energy
    float energy = result.value * S_cubed;

    // Accumulate group energy
    atomicAdd(&groupReceptorEnergies[groupIdx], energy);

    // Compute force: F = -dE/dx = -S³ * dE_grid/dx
    // The gradient from interpolation is already in physical units (per nm)
    float3 force;
    force.x = -S_cubed * result.gradient.x;
    force.y = -S_cubed * result.gradient.y;
    force.z = -S_cubed * result.gradient.z;
