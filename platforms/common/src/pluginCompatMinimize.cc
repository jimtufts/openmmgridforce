...
DEVICE void atomicAddMixed(GLOBAL mixed* RESTRICT target, const mixed value) {
        ...
        (defined(USE_MIXED_PRECISION) || defined(USE_DOUBLE_PRECISION))
    // Pre-Pascal (sm_52 Maxwell, sm_35/37 Kepler) has no hardware
    // atomicAdd(double*, double); fall back to the NVIDIA-recommended
    // 64-bit atomicCAS loop. Single precision keeps the hardware call.
    unsigned long long int* address_as_ull = (unsigned long long int*)target;
