...
// Thin wrapper: delegates to the free function in ReferenceDesolvationInterp
// so IsolatedGBSAForce GRID-mode can share the same implementation without
// coupling to this kernel class.
double ReferenceCalcGBSAGridForceKernel::interpolateReceptorHCT(
    ...
    return interpolateDesolvationGridHCT(
        desolvationGrid.get(), x, y, z, R_i_off,
        interpolationMethod, probeRadius,
        computeGrad, gradX, gradY, gradZ,
        hess, outOfBounds);
}
