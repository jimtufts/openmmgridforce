#!/usr/bin/env python
"""
Test grid generation and evaluation with tricubic interpolation using TILED mode.
This is a copy of test_tricubic.py with tiled streaming enabled.
"""
...
print("Testing grid energy with tricubic interpolation (TILED MODE)")
print("=" * 80)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    # Now load and use the grids with tricubic interpolation + tiled mode
    print("\nCreating ligand system with tricubic grid forces (TILED MODE)...")
    ...
    for i, grid_type in enumerate(['charge', 'lja', 'ljr']):
        ...
        grid_force.setInterpolationMethod(2)  # Tricubic
        ...
        print(f"  Loaded {grid_type}: interpolationMethod=2 (tricubic), invPowerMode={loaded_mode}, hasDerivatives={has_derivs}, tiledMode={grid_force.getTiledMode()}")
    ...
    print("\n" + "=" * 80)
    print("RESULTS WITH TRICUBIC INTERPOLATION (TILED MODE)")
    print("=" * 80)
    ...
    if charge_err < 3.0 and lja_err < 3.0 and ljr_err < 3.0:
        print("PASS: All energies within 3% of reference (TILED MODE)")
    else:
