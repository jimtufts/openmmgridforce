#!/usr/bin/env python
"""
Test grid generation and evaluation with triquintic interpolation and inv_power transformation.
Tests triquintic with STORED mode inv_power=-12 for LJ grids.
"""
...
print("Testing grid energy with triquintic interpolation and STORED inv_power=-12")
print("=" * 80)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for grid_type in ['charge', 'lja', 'ljr']:
        ...
        # Triquintic needs derivatives computed
        grid.setComputeDerivatives(True)

        # Use high grid cap to avoid capping artifacts with derivatives
    ...
    # Now load and use the grids with triquintic interpolation
    print("\nCreating ligand system with triquintic grid forces...")
    ...
    # Load grids and set triquintic interpolation
    for i, grid_type in enumerate(['charge', 'lja', 'ljr']):
        grid_file = os.path.join(tmpdir, f'{grid_type}.grid')
        grid_force = gfp.GridForce()
        grid_force.loadFromFile(grid_file)

        # Set triquintic interpolation (method 3)
        grid_force.setInterpolationMethod(3)
        ...
        print(f"  Loaded {grid_type}: interpolationMethod={grid_force.getInterpolationMethod()} (3=triquintic)")
    ...
    print("\n" + "=" * 80)
    print("RESULTS WITH TRIQUINTIC INTERPOLATION (STORED inv_power=-12 for LJ)")
    print("=" * 80)
    ...
    # Triquintic should have the best accuracy
    print("\n" + "=" * 80)
    ...
    # Allow up to 2% error for triquintic (should be very accurate)
