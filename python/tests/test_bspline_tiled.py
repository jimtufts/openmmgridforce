...
This is a copy of test_bspline.py with tiled streaming enabled.
"""
...
print("Testing grid energy with B-spline interpolation (TILED MODE)")
print("=" * 80)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    # Now load and use the grids with B-spline interpolation + tiled mode
    print("\nCreating ligand system with B-spline grid forces (TILED MODE)...")
    ...
    for i, grid_type in enumerate(['charge', 'lja', 'ljr']):
        ...
        print(f"  Loaded {grid_type}: interpolationMethod=1 (B-spline), tiledMode={grid_force.getTiledMode()}")
    ...
    print("\n" + "=" * 80)
    print("RESULTS WITH B-SPLINE INTERPOLATION (TILED MODE)")
    print("=" * 80)
