#!/usr/bin/env python
"""
Test grid generation and evaluation with tricubic interpolation and inv_power transformation.
Tests tricubic with STORED mode inv_power=-12 for LJ grids.
"""
...
print("Testing grid energy with tricubic interpolation and STORED inv_power=-12")
print("=" * 80)

with tempfile.TemporaryDirectory() as tmpdir:
    # Generate grids with tricubic derivatives
    print("\nGenerating grids for tricubic interpolation...")
    for grid_type in ['charge', 'lja', 'ljr']:
        ...
        # Tricubic needs derivatives computed
        grid.setComputeDerivatives(True)

        # Use default grid cap (41840.0 kJ/mol) to avoid inf derivatives at small r
        # grid.setGridCap(1e30)  # Too high - causes inf derivatives when r→0

        grid.setReceptorAtoms(receptor_atoms)
        grid.setReceptorPositionsFromLists(pos_list)

        # Use NONE mode for charge, STORED with inv_power=-12 for LJ grids
        if grid_type == 'charge':
            grid.setInvPowerMode(gfp.InvPowerMode_NONE, 0.0)
        else:  # lja and ljr
            grid.setInvPowerMode(gfp.InvPowerMode_STORED, -12.0)
    ...
    # Now load and use the grids with tricubic interpolation
    print("\nCreating ligand system with tricubic grid forces...")
    ...
    # Load grids and set tricubic interpolation
    for i, grid_type in enumerate(['charge', 'lja', 'ljr']):
        ...
        # Set tricubic interpolation (method 2)
        grid_force.setInterpolationMethod(2)
        ...
        print(f"  Loaded {grid_type}: interpolationMethod=2 (tricubic), invPowerMode={loaded_mode}, invPower={loaded_power}, hasDerivatives={has_derivs}")
    ...
    print("\n" + "=" * 80)
    print("RESULTS WITH TRICUBIC INTERPOLATION (STORED inv_power=-12 for LJ)")
    print("=" * 80)
    ...
    # Tricubic should have better accuracy than B-spline
    print("\n" + "=" * 80)
    ...
    # Allow up to 3% error for tricubic (should be very accurate)
    if charge_err < 3.0 and lja_err < 3.0 and ljr_err < 3.0:
        print("✓ PASS: All energies within 3% of reference")
    else:
